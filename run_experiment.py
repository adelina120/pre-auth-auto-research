...
def run_parallel_jobs(jobs: List[Dict], workers: int, max_steps: int, output_dir: Path) -> List[Dict]:
    ...
    with ThreadPoolExecutor(max_workers=workers) as pool:
        futures = {}
        for job in jobs:
            patient_name = f"{job.get('first_name', '')} {job.get('last_name', '')}"
            patient_id = job.get("patient_id")
            sample_type = job.get("sample_type")
            llm = job.get("llm")
            prompt = create_browser_use_prompt(BASE_URL, patient_name)
            futures[pool.submit(execute_one_patient, prompt, patient_name, patient_id, sample_type, llm, max_steps, output_dir)] = (patient_name, patient_id, sample_type, llm)

        for fut in as_completed(futures):
            patient, patient_id, sample_type, llm = futures[fut]
            try:
                results.append(fut.result())
            except Exception as e:
                results.append({"task_id": None, "patient": patient, "patient_id": patient_id, "sample_type": sample_type, "llm": llm, "error": str(e)})
    return results
...
def write_experiment_reports(task_outcomes: List[Dict], output_path: Path, llm: str, max_steps: int):
    TSV_COLUMNS = ["experiment_idx", "llm", "max_steps", "CompletionRate", "ErrorRate", "NegErrorsCount", "PosErrorsCount", "Cost", "status", "failed_reason"]
...
def write_tasks_to_json(updated_task_outcomes: List[Dict], output_path: Path, experiment_idx: int):
    ...
    tagged = [{"experiment_idx": experiment_idx, **task} for task in updated_task_outcomes]
...
if __name__ == "__main__":
    ...
    submission_results = run_parallel_jobs(
        jobs, workers=args.workers, max_steps=args.max_steps, output_dir=submission_path
    )
    
    # Fetch task results from browser use by task ID
    ...
    # Process and label output messages
